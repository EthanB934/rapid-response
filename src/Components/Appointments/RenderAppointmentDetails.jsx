...
export const RenderAppointmentDetails = ({ appointment, currentUser }) => {
  ...
  useEffect(() => {
    getVisitorByUserId(appointment.visitorId).then((res) => {
      const visitorObj = res[0];
        ...
        <>
          {currentUser.isStaff ? (
            <>
              ...
              <p className="details">
                You will be meeting with our visitor {visitor.fullName}. This
                appointment is scheduled for the following reason:{" "}
                {appointment.reason}.
              </p>
            </>
          ) : (
            <>
              ...
              <p className="details">
                ...
                {allPractices.map((practice) => {
                  if (
                    practice.id ===
                    parseInt(appointment.practitioner.practiceId)
                  )
                    return <>{practice.practice}</>;
                })}
                . He has {appointment.practitioner.experience} years of
